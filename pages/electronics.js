...
function ElectronicsPage({ newProduct, bannerImages }) {
    ...
    const bannerSticker = '/bannerSticker/Electronics_Add.webp';
    const mobileBannerSticker = '/bannerSticker/Mobile_Electronics_Add.webp';
    const bannerVideo = '/video/electronics.mp4';
    const bannerBoxTitle = 'Electronics 2023'
    const bannerBoxText = "Electronic's";
    const bannerBoxImages = '/bannerBoxImg/electronics.webp';

    return (
        <>
            <CommonHead title="Electronics Products - WishBin Store | Online Shopping site in India" />
                        ...
                        <div className='w-full h-full'>
                            ...
                            <main className="max-w-[1366px] mx-auto w-full h-full">
                                ...
                                <div className='mb-[7.5rem] mt-[7px]'>
                                    <BannerSticker bannerImg={bannerSticker} mobileBannerSticker={mobileBannerSticker} links='/electronics' bannerName='Electronics 2023' />
                                </div>
...
export const getServerSideProps = async () => {
    ...
    // Filter products
    const newProduct = product.filter(ele => ele.categoryTitle === "electronics");
    // banner image data
    const bannerImageQuery = `*[_type == 'bannerImage']`;
    const bannerImage = await client.fetch(bannerImageQuery);
    // Filter banners
    const bannerImages = bannerImage.filter(ele => ele.links === "/electronics");
