...
function GroceriesPage({ newProduct, bannerImages }) {
    ...
    const bannerSticker = '/bannerSticker/Groceries_Add.webp';
    const mobileBannerSticker = '/bannerSticker/Mobile_Groceries_Add.webp';
    const bannerVideo = '/video/groceries.mp4';
    const bannerBoxTitle = 'Fresh Groceries 2023'
    const bannerBoxText = "Groceries for Home"
    const bannerBoxImages = '/bannerBoxImg/groceries.webp';

    return (
        <>
            <CommonHead title="Groceries Products - WishBin Store | Online Shopping site in India" />
                        ...
                        <div className='w-full h-full'>
                            ...
                            <main className="max-w-[1366px] mx-auto w-full h-full">
                                ...
                                <div className='mb-[7.5rem] mt-[7px]'>
                                    <BannerSticker bannerImg={bannerSticker} mobileBannerSticker={mobileBannerSticker} links='/groceries' bannerName='Groceries Collections' />
                                </div>
...
export const getServerSideProps = async () => {
    ...
    // Filter products
    const newProduct = product.filter(ele => ele.categoryTitle === "groceries");
    // banner image data
    const bannerImageQuery = `*[_type == 'bannerImage']`;
    const bannerImage = await client.fetch(bannerImageQuery);
    // Filter banners
    const bannerImages = bannerImage.filter(ele => ele.links === "/groceries");
