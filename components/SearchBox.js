...
function SearchBox() {
  ...
  const debouncedSearch = debounce(async (searchQuery) => {
    try {
      ...
      const searched = await client.fetch(
        `*[_type == "product" && (name match $query || description match $query || categoryTitle match $query || soldBy match $query || body match $query)]`,
        { query }
      );
    ...
    <>
      ...
      >
        ...
        <button
          ...
          role="button"
          aria-label="search"
          className="cursor-pointer absolute right-0 top-0 bottom-0 h-full flex items-center justify-center bg-white dark:bg-[#101219] rounded-[20px] px-[15px] slg:py-[12px] py-[15px]"
        >
        ...
        {isModelOpen && !formik.values.query && (
          <div className="absolute max-h-[360px] -bottom-[110%] z-[9997] w-full overflow-y-auto box-border bg-white dark:bg-[#262936] p-[0.6rem_0.8rem] text-[14px] shadow-[0_2px_4px_rgba(0,0,0,0.3)] rounded-[5px]">
            No Products Found
          </div>
        )}

        {isModelOpen && !getSearchProduct.length && (
          <div className="absolute max-h-[360px] -bottom-[110%] z-[9997] w-full overflow-y-auto box-border bg-white dark:bg-[#262936] p-[0.6rem_0.8rem] text-[14px] shadow-[0_2px_4px_rgba(0,0,0,0.3)] rounded-[5px]">
            No{' '}
            <span
              dangerouslySetInnerHTML={{
                __html: highlightMatches(isSearchText),
              }}
            ></span>{' '}
            Products Found
          </div>
            ...
            {getSearchProduct.slice(0, 8).map((ele) => {
              ...
              return (
                <div
                  ...
                  key={_id}
                  className={`w-full ${
                    getSearchProduct[0] && 'pb-[10px]'
                  } p-[0.6rem_0.8rem] group cursor-pointer flex items-center gap-[8px] h-max hover:bg-[#eaeded] hover:dark:bg-[#101219]/60`}
                >
                  {/* left side */}
                  <div className="w-max">
                    {/* images */}
                  ...
                  <div className="w-full flex flex-col gap-[4px]">
                    ...
                    <p
                      ...
                      dangerouslySetInnerHTML={{
                        __html: highlightMatches(name),
                      }}
                    ></p>
                    {/* categoryTitle */}
                    <div className="w-full flex items-center justify-end">
                      <span className="w-max h-max capitalize font-[500] group-hover:shadow-[0_1px_2px_rgba(0,0,0,0.2)] p-[0.2rem_0.8rem] bg-[#E7EDEF] dark:bg-[#101219] text-[12px] rounded-[10px]">
                        {categoryTitle}
                      </span>
                    </div>
                  </div>
